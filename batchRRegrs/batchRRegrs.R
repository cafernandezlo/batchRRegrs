# ======================================================================
# simpleRRegrs - R Regressions made simple!!
# ======================================================================
...
RemNear0VarCols <- function(ds,fDet=FALSE,fOutVal = FALSE, outFile="ds3.No0Var.csv") {
  #================================================
  ...
  if (fOutVal==T){
    return(list('train'= ds.Rem0NearVar,'ds.var'=ds.var))
  }else return(as.data.frame(ds.Rem0NearVar)) # return the new data frame without near zero variance
}
#----------------------------------------------------------------------------------------------------------------------
...
RemCorrs <- function(ds,fDet,cutoff,outFile) {
  ...
  if (length(highlyCor) == 0){
    return (list('ds'=ds,'highlyCor'=FALSE))
  }
  ...
  return(list('ds'=as.data.frame(DataSetFiltered.scale),'highlyCor'=TRUE,'corrColums'=highlyCor))
}
#----------------------------------------------------------------------------------------------------------------------

DsSplit <- function(ds,ds.val,trainFrac=3/4,fDet=FALSE,fOutVal=FALSE,PathDataSet="",iSeed) {
  # ===============================================
  ...
  my.datf.test <- my.datf[-inTrain,]           # TEST dataset frame
    ...
    my.datf.train<- ds                         # TRAIN dataset frame         
    my.datf.test <- ds.val                     # TEST dataset frame
  }
...
LMreg <- function(my.datf.train,my.datf.test,sCV,iSplit=1,fDet=F,outFile="") {
  ...
  output.tr <- cbind(pred.tr,my.datf.train[,1])
  output.ts <- cbind(pred.ts,my.datf.test[,1])
  
  write.table(output.tr,sep=',',file=paste(outFile,'.split.',iSplit,'.vectors.tr.csv',sep=''))
  write.table(output.ts,sep=',',file=paste(outFile,'.split.',iSplit,'.vectors.ts.csv',sep=''))
  write.table(my.stats,sep=',',file=paste(outFile,'.split.',iSplit,'.my.stats.csv',sep=''))
  save(lm.fit,file=paste(outFile,'.split.',iSplit,'.lm.model.RData',sep='')) 
...
GLMreg <- function(my.datf.train,my.datf.test,sCV,iSplit=1,fDet=F,outFile="") {
  ...
  write.table(output.ts,sep=',',file=paste(outFile,'.split.',iSplit,'.vectors.ts.csv',sep=''))
  write.table(my.stats,sep=',',file=paste(outFile,'.split.',iSplit,'.my.stats.csv',sep=''))
  save(glm.fit,file=paste(outFile,'.split.',iSplit,'.glm.model.RData',sep='')) 
...
PLSreg <- function(my.datf.train,my.datf.test,sCV,iSplit=1,fDet=F,outFile="") {
  ...
  write.table(output.ts,sep=',',file=paste(outFile,'.split.',iSplit,'.vectors.ts.csv',sep=''))
  write.table(my.stats,sep=',',file=paste(outFile,'.split.',iSplit,'.my.stats.csv',sep=''))
  save(pls.fit,file=paste(outFile,'.split.',iSplit,'.pls.model.RData',sep='')) 
...
LASSOreg <- function(my.datf.train,my.datf.test,sCV,iSplit=1,fDet=F,outFile="") {
  ...
  write.table(output.ts,sep=',',file=paste(outFile,'.split.',iSplit,'.vectors.ts.csv',sep=''))
  write.table(my.stats,sep=',',file=paste(outFile,'.split.',iSplit,'.my.stats.csv',sep=''))
  save(las.fit,file=paste(outFile,'.split.',iSplit,'.las.model.RData',sep='')) 
...
SVRMreg <- function(my.datf.train,my.datf.test,sCV,iSplit=1,fDet=F,outFile="",cs=c(1,5,10,15,20)) {
  ...
  write.table(output.ts,sep=',',file=paste(outFile,'.split.',iSplit,'.vectors.ts.csv',sep=''))
  write.table(my.stats,sep=',',file=paste(outFile,'.split.',iSplit,'.my.stats.csv',sep=''))
  save(svmL.fit,file=paste(outFile,'.split.',iSplit,'.svmL.model.RData',sep='')) 
...
NNreg <- function(my.datf.train,my.datf.test,sCV,iSplit=1,fDet=F,outFile="") {
  ...
  nn.fit<- train(net.c~.,data=my.datf.train,
                 ...
                 #tuneGrid=expand.grid(.size=c(600,1000),.decay=c(0,0.01,0.2,0.1)))
                 tuneGrid=expand.grid(.size=c(200,300,400),.decay=c(0,0.01,0.2,0.1)))
                 #tuneGrid=expand.grid(.size=c(200,400,600,1000),.decay=c(0,0.01,0.2,0.1)))
  ...
  write.table(output.ts,sep=',',file=paste(outFile,'.split.',iSplit,'.vectors.ts.csv',sep=''))
  write.table(my.stats,sep=',',file=paste(outFile,'.split.',iSplit,'.my.stats.csv',sep=''))
  save(nn.fit,file=paste(outFile,'.split.',iSplit,'.nn.model.RData',sep='')) 
...
RFreg <- function(my.datf.train,my.datf.test,sCV,iSplit=1,fDet=F,outFile="") {
  ...
  rf.fit<- train(net.c~.,data=my.datf.train,
                 ...
                 metric='RMSE',ntree=50,tuneGrid =tuneParam)
  ...
  write.table(output.ts,sep=',',file=paste(outFile,'.split.',iSplit,'.vectors.ts.csv',sep=''))
  write.table(my.stats,sep=',',file=paste(outFile,'.split.',iSplit,'.my.stats.csv',sep=''))
  save(rf.fit,file=paste(outFile,'.split.',iSplit,'.rf.model.RData',sep='')) 
...
SVMRFEreg <- function(my.datf.train,my.datf.test,sCV,iSplit=1,fDet=F,outFile="",cs=c(1,5,10,15,20),eps=c(0.01,0.1,0.3)) {
  ...
  write.table(output.ts,sep=',',file=paste(outFile,'.split.',iSplit,'.vectors.ts.csv',sep=''))
  write.table(my.stats,sep=',',file=paste(outFile,'.split.',iSplit,'.my.stats.csv',sep=''))
  save(rfesvm.fit,file=paste(outFile,'.split.',iSplit,'.rfesvm.model.RData',sep='')) 
...
RFRFEreg <- function(my.datf.train,my.datf.test,sCV,iSplit=1,fDet=F,outFile="") {
  ...
  write.table(output.ts,sep=',',file=paste(outFile,'.split.',iSplit,'.vectors.ts.csv',sep=''))
  write.table(my.stats,sep=',',file=paste(outFile,'.split.',iSplit,'.my.stats.csv',sep=''))
  save(rferf.fit,file=paste(outFile,'.split.',iSplit,'.rferf.model.RData',sep='')) 
...
ENETreg <- function(my.datf.train,my.datf.test,sCV,iSplit=1,fDet=F,outFile="") {
  ...
  write.table(output.ts,sep=',',file=paste(outFile,'.split.',iSplit,'.vectors.ts.csv',sep=''))
  write.table(my.stats,sep=',',file=paste(outFile,'.split.',iSplit,'.my.stats.csv',sep=''))
  save(enet.fit,file=paste(outFile,'.split.',iSplit,'.enet.model.RData',sep='')) 
...
simpleRRegrs <- function(DataFileName="ds.House.csv",DataFileSep=",",PathDataSet="DataResults",noCores=1,
                   OutValidationFile='ds.validation.csv',fOutVal='F',
  ...
  Params.df = rbind(Params.df,data.frame(RRegrs.Parameters="OutValidationFile",Parameter.Value=as.character(OutValidationFile),Description="External validation file"))
  ...
  Params.df = rbind(Params.df,data.frame(RRegrs.Parameters="fOutVal",Parameter.Value=as.character(fOutVal),Description="If uses external file for validation"))
  ...
  fOutVal      = as.logical(fOutVal)      # flag to use an external data file for validation
  ...
  sDescription=paste("=======================================================================================================",
                     "simpleRRegrs - R Regression Models made simple!",
                     "Get the best regression models for one dataset using R caret methods","AUTHORS:",
                     "Cristian R. Munteanu: RNASA-IMEDIR, University of A Coruna, Spain, muntisa@gmail.com",
                     "Carlos Fernandez-Lozano: RNASA-IMEDIR, University of A Coruna, Spain, carlos.fernandez@udc.es",
                     "=======================================================================================================",sep="\n")
  ...
  # -----------------------------------
  # (1.3) Load the VALIDATION EXTERNAL DATASET
  # -----------------------------------
  
  if(fOutVal==T){
    # Generate path + file name = original dataset
    if (file.exists(OutValidationFile)) { # is it a full path already?
      extFile <- OutValidationFile
    } else {
      extFile <- file.path(PathDataSet, OutValidationFile)
    }
    # -----------------------------------
    # (1.2) Load the EXTERNAL DATASET
    # -----------------------------------
    cat("\n-> Loading external validation dataset ...\n")  # it can contain errors, correlations, near zero variance columns
    cat("      ---> ",extFile,"\n")
    ds.dat0 <- read.csv(extFile,header=T,sep=DataFileSep)          # original dataset frame
    ...
    net.c<- as.numeric(as.character(net.c)) # values
    # full ds frame with training and test
    ds.val<- as.data.frame(cbind(net.c,t(ds.dat1)))
  ...
  if(fOutVal==T){
    if(length(which(is.na(ds.val)==TRUE))!=0){
      cat("-> Removal of NA values ...\n")
      outFile <- file.path(PathDataSet,"ds.NoNA.OutValidationFile.csv") # the same folder as input  
      
      # get the ds without NA values-- currently use the default function (mean) 
      ds.val <- impute.funct(ds.val)
      if (fDet == TRUE){   # write as details the corrected ds file
        write.csv(ds.val, outFile,row.names=F, quote=F)}
    }
  }
  ...
  if (fRemNear0Var==T) {
    ...
    if(fOutVal==T){
      lista<-RemNear0VarCols(ds[,2:dim(ds)[2]],fDet,fOutVal,outFile)
      ds<-cbind("net.c" = ds[,1],lista$train)
      ds.val.new <- ds.val[,2:dim(ds.val)[2]]
      ds.val <- cbind('net.c' = ds.val[,1],ds.val.new[,-(lista$ds.var)])
    }else{
      ds <- cbind("net.c" = ds[,1],RemNear0VarCols(ds[,2:dim(ds)[2]],fDet,fOutVal,outFile))
    }
  }
  ...
  if (fScaling==T) {
    ...
    outValFile <- file.path(PathDataSet,"ds.scaled.externalValidation.csv")
    # run fuction for scaling input dataset file
    ds <- ScalingDS(ds,iScaling,iScalCol,fDet,outFile)
    ds.val <- ScalingDS(ds.val,iScaling,iScalCol,fDet,outValFile)
  ...
  if (fRemCorr==T) {    
    ...
    lista<-RemCorrs(ds[,2:dim(ds)[2]],fDet,cutoff,outFile)
    
    if(fOutVal==T){
      if(lista$highlyCor == TRUE){
        ds<-cbind("net.c" = ds[,1],lista$ds)
        ds.val.new <- ds.val[,2:dim(ds.val)[2]]
        ds.val <- cbind('net.c' = ds.val[,1],ds.val.new[,-(lista$corrColums)])
      }else{
        ds<-lista$ds
        ds.val <- ds.val
      }
  ...
  for (i in 1:iSplitTimes) {   # Step splitting number = i
    ...
    dsList  <- DsSplit(ds,ds.val,trainFrac,fDet,fOutVal,PathDataSet,iSeed) # return a list with 2 datasets = dsList$train, dsList$test
